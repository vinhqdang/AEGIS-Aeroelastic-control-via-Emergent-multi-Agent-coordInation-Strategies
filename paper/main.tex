...
and the Exploration Scale That Hides It}
...
\item A validated aeroelastic testbed and a five-rung classical baseline ladder
...
\item A negative result we consider as informative as the positive one: the
communication and action-parameterisation mechanisms we derived from the same
physics do not help, and a plain feedforward policy with the same credit signal
significantly outperforms all of them.
\item The observation that the exploration scale, not the learning algorithm,
sets performance on this problem, and that it hides both results above until it
is matched to the actuation amplitude that damps the wing.
\end{itemize}
...
The plant above buys speed with two approximations that a reviewer is entitled to
distrust: two-dimensional strip aerodynamics, and quasi-steady flap derivatives
that neglect the flap's own shed wake. We quantified both against SHARPy
\cite{delcarre2019sharpy}, which couples an unsteady vortex-lattice method to a
geometrically exact beam.

The comparison is unusually clean because SHARPy's own Goland template carries
structural data identical to ours -- $GJ=\SI{0.987581e6}{\newton\metre\squared}$,
$EI=\SI{9.77221e6}{\newton\metre\squared}$, $m=\SI{35.71}{\kilo\gram\per\metre}$,
$J=\SI{8.64}{\kilo\gram\metre}$, elastic axis at \SI{33}{\percent} and centre of
gravity at \SI{43}{\percent} chord. Any disagreement is therefore a disagreement
about the aerodynamics.

\paragraph{Flutter point.} At sea level SHARPy places flutter at
\SI{154.46}{\metre\per\second} and \SI{69.3}{\radian\per\second}, against
\SI{137.35}{\metre\per\second} and \SI{69.34}{\radian\per\second} for our model.
The \emph{frequency} agrees to \SI{0.06}{\percent}, so both models identify the
same bending--torsion coalescence; only the critical speed differs, by
\SI{-11.1}{\percent}. The direction is the one two-dimensional aerodynamics
predicts: strip theory has no tip relief, over-predicts loading near the tip and
therefore under-predicts the flutter speed. Our plant is thus conservative, which
is the safe direction for a control study -- the policies were trained against a
harder problem than reality.

\paragraph{Control authority.} This one goes the other way and is larger. A
part-span flap loses effectiveness to spanwise carryover and to the flow around
its ends, neither of which a sectional derivative can represent. Integrating our
thin-airfoil $C_{l\delta}=3.826$ per radian over the flapped span (\SI{25}{\percent}
of span, read from the generated aerodynamic grid) gives a whole-wing
$\mathrm{d}C_L/\mathrm{d}\delta$ of $0.957$ per radian, against $0.495$ from
UVLM on the finer of two grids. Our plant therefore over-estimates control
authority by a factor of $1.9$, grid-converged to within \SI{3}{\percent}
between $M{\times}N = 8{\times}16$ and $16{\times}32$.

\paragraph{What this does and does not invalidate.} Every controller in this
study, classical and learned, was evaluated on the same plant and so received the
same inflated authority. Relative comparisons, ablations and significance tests
are therefore unaffected. Absolute decay rates would not transfer unchanged to a
UVLM plant, and we do not claim they would. The two discrepancies also act in
opposite directions -- a harder flutter problem, an easier control problem -- so
they cannot be folded into a single safety factor, and we report them separately.
...
\texttt{--} marks conditions infeasible for any controller with these actuators.}
\label{tab:main}
\footnotesize
\input{results_table}
\end{table}
...
\begin{table}[t]
\centering
\caption{Architecture ablation at $\sigma=0.05$, three seeds per variant. Every
mechanism we proposed degrades performance relative to a plain policy with the
same credit signal.}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\textwidth]{figures/fig_exploration.pdf}
\caption{Energy decay rate against initial exploration standard deviation, all
else held fixed. The red line marks the deflection amplitude the classical
controller actually uses. Two seeds per point.}
...
\begin{itemize}
\item The learned policies do not match the decentralised classical controller
on average nominal damping ($-1.53$ against $-2.05$ per second). They are more
uniform across the envelope and hold a condition where it diverges, but we do not
claim they dominate it.
\item The robustness result is scale-dependent. At $\sigma=0.30$ the learned
policies held all six feasible conditions, matching a fault-aware oracle; at
$\sigma=0.05$, where damping is three times better, they hold five. We report the
trade-off rather than the better half of it.
\item The credit signal requires the modal control influence $v_r^{\top} b_k$.
This is free in simulation but on hardware needs a modal model, and robustness to
error in $B$ is untested.
\item The plant over-estimates control authority by a factor of $1.9$ relative to
UVLM (Section~\ref{sec:sharpy}). Relative results are unaffected, since every
controller faced the same plant, but absolute decay rates would not transfer. We
have not retrained or re-evaluated policies on the UVLM plant itself.
\item Proposition~\ref{prop:diff} concerns the difference reward, not optimality
of the resulting policy.
\item Ablations use six seeds for the credit comparison and three for the
architecture comparison, on a three-surface wing. The eight-surface results are
suggestive but were not run to the same statistical standard.
\item The exploration-scale finding is established on this plant and this
algorithm. We expect it to generalise to control tasks whose useful actuation
band is narrow relative to the action range, but we have not shown that.
\end{itemize}
